...
class AppStore extends Component {
  state = {selectedTab: tabsList[0].tabId, searchInput: ''}
  ...
  render() {
    ...
    const filteredAppsCategoryList = appsList.filter(
      each => each.category === selectedTab,
    )
    const filteredAppsList = filteredAppsCategoryList.filter(each =>
        ...
        <ul className="tabs-container">
          {tabsList.map(each => (
            <TabItem
              ...
              isSelectedTab={each.tabId === selectedTab}
            />
          ))}
        </ul>
